...
- Tag highlighting in the Markdown editor is built in, see [Styling inline tags](#styling-inline-tags). [Rich Markdown](https://github.com/CalebJohn/joplin-rich-markdown) (version ≥ 0.14) can still do it, and the setup below is kept for reference, but if you use both you will get two backgrounds on every tag. Remove the `rm-tag` entry and its stylesheet rule, or turn off `Inline tags: Highlight in editor`.
