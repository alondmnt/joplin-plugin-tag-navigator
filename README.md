...
- Tag highlighting in the Markdown editor is built in, see [Styling inline tags](#styling-inline-tags). [Rich Markdown](https://github.com/CalebJohn/joplin-rich-markdown) (version ≥ 0.14) can still do it, and the setup below is kept for reference, but the two overlap: both style the same tags in the same editor. Pick one, either by removing the `rm-tag` entry and its stylesheet rule or by turning off `Inline tags: Highlight in editor`.
